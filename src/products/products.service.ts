        ...
        const presentations = product.product_acabados.flatMap(pa =>
          pa.acabados.presentations.map(pres => {
            ...
            const discount = discounts.find(
              d => d.presentacion_id === pres.id && d.producto_id === product.id,
            );
            ...
            return {
              ...
              imagen: imageEntry?.imagen || null,
              orden: pres.product_presentation_images[0]?.orden || null,
            };
          }),
        ).filter(p => p.valor > 0);

        const presentationWeb = presentations.find(p => p.id === product.id_presentacion_web);
        const presentationWithDiscount = presentations.find(p => p.descuento === true);

        const preferredPresentation = presentationWithDiscount ?? presentationWeb;

        result.push({
          id_origen: product.id,
          token: product.token,
          nombre: product.nombre,
          image: product.image,
          ...
          valor: preferredPresentation?.valor ?? 0,
          ...
          alto: product.alto?.toString() ?? null,
          grosor: product.grosor?.toString() ?? null,
          ...
          clasifica_collections: product.clasifica_collections
            ? {
              id: product.clasifica_collections.id.toString(),
              nombre: product.clasifica_collections.nombre,
            }
            : null,
